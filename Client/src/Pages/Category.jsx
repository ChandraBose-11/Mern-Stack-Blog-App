...
//         const res = await fetch(`https://mern-stack-blog-app-8.onrender.com/api/post/getPosts?category=${categoryQuery}`,{
//           credentials:"include"
//         });
...
const Category = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const categoryQuery = categoryName.replace(/-/g, " ");
        // ✅ FIXED: getposts (lowercase) to match backend
        const res = await fetch(
          `https://mern-stack-blog-app-8.onrender.com/api/post/getposts?category=${categoryQuery}`,
          {
            credentials: "include",
          }
        );
