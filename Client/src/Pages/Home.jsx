...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      // ✅ FIXED: getposts (lowercase)
      const res = await fetch(
        `https://mern-stack-blog-app-8.onrender.com/api/post/getposts?startIndex=0&limit=8`,
        { credentials: "include" }
      );
      const data = await res.json();
      setPosts(data.posts || []);
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50 dark:bg-[#0f172a] transition-colors duration-500 overflow-hidden">
        ...
        <section className="relative flex flex-col justify-center items-center text-center min-h-[85vh] h-full pt-28 pb-16 sm:h-[90vh] px-6 sm:px-10 md:px-16 overflow-hidden">
          ...
          <div className="relative z-10 max-w-4xl px-4">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <a
                href="/search"
                className="px-8 py-3 rounded-full bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold shadow-lg hover:scale-105 duration-300"
              >
                Explore Posts
              </a>
              <a
                href="/about"
                className="px-8 py-3 rounded-full border-2 border-pink-600 dark:border-purple-500 text-pink-600 dark:text-purple-400 font-semibold hover:bg-gradient-to-r hover:border-transparent from-purple-500 to-pink-600 hover:text-white duration-300"
              >
                Learn More
              </a>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8">
            {categories.map((cat) => (
              <a
                key={cat.name}
                href={cat.link}
                className={`flex flex-col items-center justify-center p-6 rounded-2xl text-white font-semibold shadow-xl hover:scale-105 duration-300 ${cat.color}`}
              >
                <div className="text-4xl mb-2">{cat.icon}</div>
                <span className="text-sm">{cat.name}</span>
              </a>
        ...
        <section className="bg-white dark:bg-[#0f172a] py-10 px-6 sm:px-10 md:px-16">
          <div className="max-w-6xl mx-auto text-center">
            <h2 className="text-3xl font-extrabold font-serif mb-8 text-gray-900 dark:text-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
              {feature.map((feature, i) => (
                <div
                  key={i}
                  className={`p-10 rounded-2xl bg-gradient-to-br ${feature.color} text-white shadow-xl hover:scale-[1.03] duration-300`}
                >
                  <h3 className="font-semibold text-xl mb-3">{feature.title}</h3>
                  <p>{feature.text}</p>
                </div>
